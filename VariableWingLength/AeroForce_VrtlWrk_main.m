function [Force, Moment] = AeroForce_VrtlWrk_main(t, xm, param)
...
%     dF_L = LiftForce*transpose(RG2rw)*[0;sign(vRL(2))*sin(alpha);-sign(vRL(3))*cos(alpha)];
    ...
    Me_R(1) = Me_R(1) + dot(-Rz(-0)*Ry(-0)*dRx*Rx(-phi)*transpose(Rx(qx)*Ry(qy)*Rz(qz))*[r;sign(vRL(2))*param.b*1/4*coeff1;0],dFe);
