function F_int = AeroForce_InducedVelocity_Intrnl(t, xm, param)
...
%     dwlG = transpose(RG2lw)*(dwlL);         % transfers the local angular velocity to global
    vR = [0;dy;dz] +V_in * k_i * [0;cos(phi);sin(phi)] + cross(dwrG,transpose(RG2rw)*[r;0;0]);  % vR in global coordinates
...
%     F_int(1:3) = F_int(1:3) + cross((Rz(-qz)*Ry(-qy)*Rx(-qx))*[r;sign(vRL(2))*param.b*1/4*coeff1;0],dFe_B);    
%     F_int(2) = F_int(2) + cross((Rz(-qz)*Ry(-qy)*Rx(-qx))*[r;sign(vRL(2))*param.b*1/4*coeff1;0],dFe_B);    
%     F_int(3) = F_int(3) + cross((Rz(-qz)*Ry(-qy)*Rx(-qx))*[r;sign(vRL(2))*param.b*1/4*coeff1;0],dFe_B);        
%     F_int(4) = 0;
    ...
    F_in = cos(phi)*Fe_R(2) + sin(phi)*Fe_R(3);  % normal to the actuator disk (force applied to the wing)
    F_in  = 2 * F_in;  % to account for two wings positive poiting to the head
    
    V_in = sqrt(abs(F_in)/(2*param.Rof*(pi*wl^2)))*(sign(F_in)); % induced velocity pointing to the head
    
end % The end of the induced velocity loop
