...
warning ('off');
%       phi0        V_frwd      Vz       dphi0       f       flp     pro    swp    pro_shft   swp_shft  pro_phs
x_0 = [90*pi/180    0.00       0.00    0*pi/180      45      58     -45     0        0          0];  % for hovering constant wing span wl=12.5, f=20
x_0 = [1.5786      0.0725    -0.0000   -1.6628    49.9811    58.    -35    0       0            0];  % for hovering constant wing span wl=12.5, f=20
x_0 = [1.5786     0.0385     0.0000   -1.1209    51.0214     58.    -43    0       0            0];  % for hovering constant wing span wl=12.5, f=20


% state space
x_0 = [1.5966    0.0442    0.0002   -1.3995      33.0154     89.   -45    0       0       0    0];  % for hovering constant wing span wl=12.5, f=20
x_0 = [1.5959    0.0442    0.0002   -1.3936      33.3910     88    -45    0       0       0    0];
x_0 = [1.5937    0.0439    0.0001   -1.3744      34.5710     85    -45    0       0       0    0];
x_0 = [1.5903    0.0431    0.0001   -1.3375      36.7350     80    -45    0       0       0    0];
...
% State_space = [1.2923    0.0556   -0.0049    0.9788   59.9589];
% State_space = [1.57935334885116,0.0741910250153231,-7.80923260715284e-06,-1.70166157996832,48.3067352563492];
...
% State_space = [0.00     0.00      0.0          50      0.00          0.00];
...
%              V_frwd      Vz       dphi0       f      pro_shft      pro_phs   
State_space = [0.00     0.00      0.0          10      0.00          0.00];

% if StartFlag ==1
%     if exitflag ~= 1 && flpAng == 90
%         State_space(4) = 10;
%     end
% else
%     StartFlag = 1;
...
A = ones(TimeGrid-1,1);
LB = [-1*A; -1*A; -2.0*pi/2*A; -10*A; -10*A; -1000*A; 1; -45; -45];
UB = [1*A;  1*A;  2.0*pi/2*A;  10*A;  10*A;  1000*A; 100; 45; 45];
tic
...
OutputData = [OutputData;[Mb State_space Control_param output.constrviolation exitflag]];

% end
end
save 3-12-2016-Bat-Glasso-Mw10-v2
end
end
% end
